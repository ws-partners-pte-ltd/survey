        ...
        function App() {
            ...
            const renderStart = () => (
                <div className={`transition-all duration-1000 transform ${isAnimating ? 'opacity-0 scale-95' : 'opacity-100 scale-100'} flex flex-col items-center justify-center w-full max-w-6xl mx-auto px-6 flex-1 text-center`}>
                    ...
                    <p
                        style={{ textShadow: '0 1px 2px rgba(0,0,0,0.95), 0 0 14px rgba(0,0,0,0.7), 0 0 28px rgba(0,0,0,0.45)' }}
            ...
            const renderOrientation = () => (
                <div className={`transition-all duration-700 ease-in-out transform ${isAnimating ? 'opacity-0 translate-x-10' : 'opacity-100 translate-x-0'} w-full max-w-3xl mx-auto px-6 sm:px-10 flex-1 flex flex-col justify-center`}>
                    <div className="w-full">
                        ...
                        <h2 className="text-xl md:text-2xl font-normal text-art-text mb-4 md:mb-5 leading-snug tracking-wide">
                            このサーベイの回答方法
                        </h2>
                        <p className="text-sm md:text-base text-art-muted leading-relaxed mb-5">
                            各設問では、5段階の選択肢の中から
                            <span className="text-art-accentYellow font-bold">「現状」</span>と
                            <span className="text-art-accentPeach font-bold">「3年後の理想」</span>
                            を1つずつ選んでいただきます。
                        </p>
                        {/* 視覚的サンプル */}
                        <div className="border border-art-border/50 px-4 md:px-6 py-4 md:py-5 mb-5">
                            <div className="text-[10px] tracking-widest uppercase text-art-muted/70 mb-3">例：「幹部層の構成」の場合</div>
                            <div className="flex items-center justify-end gap-3 mb-2 text-[10px] tracking-widest uppercase">
                                <span className="w-10 text-center text-art-accentYellow">現状</span>
                                <span className="w-10 text-center text-art-accentPeach">3年後</span>
                            </div>
                            <div className="space-y-1.5 text-xs md:text-sm">
                                {[
                                    { txt: "1. 日本人駐在員が中心となり…", cur: false, ideal: false },
                                    { txt: "2. 日本人が主軸を担いつつ…", cur: true, ideal: false, note: "← 現状はこちら" },
                                    { txt: "3. 部門の特性に応じて…", cur: false, ideal: false },
                                    { txt: "4. 多くの主要ポストを現地人材…", cur: false, ideal: true, note: "← 3年後の理想はこちら" },
                                    { txt: "5. 社長を含め、全主要ポスト…", cur: false, ideal: false },
                                ].map((row, i) => (
                                    <div key={i} className="flex items-center gap-3">
                                        <span className="flex-1 text-art-muted">{row.txt}</span>
                                        {row.note && <span className="text-[10px] text-art-muted/70 hidden md:inline">{row.note}</span>}
                                        <span className={`w-5 h-5 md:w-6 md:h-6 rounded-full border flex items-center justify-center text-[9px] ${row.cur ? 'bg-art-accentYellow border-art-accentYellow text-art-bg font-bold' : 'border-art-border'}`}>{row.cur ? '■' : ''}</span>
                                        <span className={`w-5 h-5 md:w-6 md:h-6 rounded-full border flex items-center justify-center text-[9px] ${row.ideal ? 'bg-art-accentPeach border-art-accentPeach text-art-bg font-bold' : 'border-art-border'}`}>{row.ideal ? '◆' : ''}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        <p className="text-xs md:text-sm text-art-muted/80 italic mb-6">
                            ※ 深く悩まず、最初に思い浮かんだ感覚でお選びください。
                        </p>
                        <div className="flex justify-end">
                            <button onClick={() => { if (isAnimating) return; setIsAnimating(true); setTimeout(() => { setShowOrientation(false); setCurrentStep(0); setIsAnimating(false); }, 600); }} className="px-10 py-3 border border-art-accentYellow text-art-accentYellow hover:bg-art-accentYellow hover:text-art-bg transition-all uppercase tracking-widest text-sm">
                                回答を始める
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="w-full">
                        ...
                        {q.type === 'text' && (
                            <div className="w-full">
                                ...
                                {CONFIG !== ERROR_CONFIG && (
                                    <div className="mt-3 md:mt-4 flex justify-end">
                                        <button onClick={handleNext} disabled={!answers[q.id]?.trim()} className="px-8 py-2.5 border border-art-text text-art-text hover:bg-art-text hover:text-art-bg transition-all disabled:opacity-20 uppercase tracking-widest text-sm">NEXT</button>
                                    </div>
                                )}
                            </div>
                        )}
            ...
            return (
                <div className="min-h-screen relative flex flex-col w-full">
                    ...
                    <main className="flex-1 flex flex-col justify-center relative z-10 min-h-screen w-full py-10 md:py-12">
                        {currentStep === -1 && !showOrientation && renderStart()}
                        ...
                        {currentStep === QUESTIONS.length && (
                            <div className="flex-1 flex flex-col items-center justify-center px-6 text-center">
                                ...
                                <div className="flex flex-col items-center gap-8">
                                    <button onClick={handleSubmit} disabled={isSubmitting} className="px-16 py-5 border border-art-accentGreen text-art-accentGreen hover:bg-art-accentGreen hover:text-art-bg transition-all text-xl tracking-widest uppercase disabled:opacity-50">
